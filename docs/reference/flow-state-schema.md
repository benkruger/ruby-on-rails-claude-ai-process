...
| `_stop_instructed` | boolean | Discussion-mode flag. Set to `true` by the Stop hook (`check_first_stop`) on the first stop event during an active flow — either a pure discussion-mode block (no pending) or a conditional continuation (pending set, with user-message awareness). On subsequent stops the flag is already `true` and `check_first_stop` allows stop through to `check_continue`. Cleared by `check_continue` when `_continue_pending` is consumed (atomically in the same `mutate_state` closure), by `check_halt_pending` when a continue token clears `_halt_pending` (cleared in the same `mutate_state` call to prevent residue from bleeding into subsequent phases), and by `phase_enter()` on phase entry. Not cleared on session mismatch — `phase_enter()` handles the reset when the new session enters a phase. Transient. |
| `_halt_pending` | boolean | Halt-pause flag for the explicit-user-pause-directive contract. Set to `true` by the Stop hook (`check_halt_pending`) when the user has typed a prose message after the model's most recent Skill action AND the message contains no continue token (`continue`, `resume`, `proceed`, `go ahead`, `keep going`). Cleared in the same predicate when the user types a continue token (along with `_stop_instructed`) and when the current phase is no longer in-progress OR no longer configured `auto` (stale halt residue from a prior phase). Default-false on missing or wrong-type values. Persists across multiple Stop events — every subsequent Stop continues to block until a continue token arrives. `_continue_pending` is preserved across every set and clear so the multi-child-skill resume path can read it once the halt is cleared. See `.claude/rules/autonomous-phase-discipline.md` "Mechanical halt-pause contract". Transient. |
