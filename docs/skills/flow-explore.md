...
| 5 | Wrap-up | Capture What/Why/Acceptance, validate with `--mode vanilla`, file without `--label decomposed`, clear marker |
...
5. **Step 5 — Wrap-up:** Captures the agreed problem statement (What/Why/Acceptance Criteria), runs the backwards-reasoning and include-bias scans, presents the draft inline, asks for filing approval via `AskUserQuestion`, validates the body via `bin/flow validate-issue-body --mode vanilla`, and files the issue via `bin/flow issue` without the `decomposed` label. The vanilla issue stays as the canonical problem statement.
...
- **Step 1 Conversation Gate** — `/flow:flow-explore` invoked without a topic argument clears the marker and stops with usage guidance. No interactive prompt; the user re-runs the command with `<topic>`.
...
- **Step 5 File Gate** — `AskUserQuestion` mandatory before filing. The validator gate fires on the body before `bin/flow issue` runs; a body that fails `--mode vanilla` validation routes back to the Revise loop.
