...
| 6 | Wrap-up | `decompose:decompose` invocation, transform to Implementation Plan wrapped in FLOW-PLAN sentinels, validate with `--mode decomposed`, file with `--label decomposed`, close parent via `bin/flow close-issue --comment`, clear marker |
