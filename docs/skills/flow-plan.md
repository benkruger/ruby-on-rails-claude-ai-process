...
- **Step 6 Plan Review Gate** — The drafted plan is audited by the cognitively isolated `flow:plan-reviewer` agent against the `.claude/rules/` corpus. A `re-decompose` verdict triggers up to three re-decompose attempts; if all three are exhausted the issue is filed with the last drafted plan and the violations are surfaced to the user as a non-blocking advisory warning. The reviewer advises — it never blocks filing.
