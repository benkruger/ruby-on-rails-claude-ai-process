...
```bash
echo "$(date -u +%Y-%m-%dT%H:%M:%SZ) [Phase 2] Step X — desc — START" >> /tmp/flow-<branch>.log; COMMAND; EC=$?; echo "$(date -u +%Y-%m-%dT%H:%M:%SZ) [Phase 2] Step X — desc — DONE (exit $EC)" >> /tmp/flow-<branch>.log; exit $EC
```
