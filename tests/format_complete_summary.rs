...
/// Branch 1: one phase carries one skipped agent — the Skipped
/// Agents section renders a header and a single entry line.
/// Regression guard: if `skipped_agents_section` stops rendering, a
/// review agent that flow-review or flow-learn could not run becomes
/// invisible in the Complete banner.
#[test]
fn summary_skipped_agents_single_phase_single_agent() {
    let mut state = all_complete_state();
    state["phases"]["flow-review"]["agents_skipped"] = json!([
        {
            "agent": "reviewer",
            "reason": "rate_limit",
            "timestamp": "2026-01-01T00:30:00-08:00",
    ...
    assert!(result.summary.contains("Skipped Agents"));
    assert!(result.summary.contains("reviewer — rate_limit (Review)"));
}

/// Branch 2: multiple phases each carry skipped agents, and one
/// phase carries more than one — every entry renders, grouped by
/// phase in PHASE_ORDER. Regression guard: a per-phase or per-entry
/// loop bug would drop entries from the banner.
#[test]
fn summary_skipped_agents_multiple_phases_and_agents() {
    let mut state = all_complete_state();
    state["phases"]["flow-review"]["agents_skipped"] = json!([
        {
            "agent": "reviewer",
    ...
    state["phases"]["flow-learn"]["agents_skipped"] = json!([
        {
            "agent": "learn-analyst",
    ...
    assert!(result.summary.contains("Skipped Agents"));
    assert!(result.summary.contains("reviewer — rate_limit (Review)"));
    assert!(result.summary.contains("adversarial — api_error (Review)"));
    assert!(result
        .summary
        .contains("learn-analyst — exhausted_retries (Learn)"));
}

/// Branch 3: no phase carries `agents_skipped` — the section is
/// omitted entirely. Regression guard: an unconditional header
/// render would print an empty Skipped Agents block on every clean
/// flow.
#[test]
fn summary_skipped_agents_omitted_when_none() {
    let state = all_complete_state();
    let result = format_complete_summary(&state, None);
    assert!(!result.summary.contains("Skipped Agents"));
}

/// Branch 4: a malformed `agents_skipped` value (wrong type on one
/// phase, entries missing `agent` or `reason` on another) is
/// tolerated — `format_complete_summary` does not panic and the
/// recoverable entries still render. Regression guard: a hand-edited
/// or corrupted state file must not crash the Complete banner.
#[test]
fn summary_skipped_agents_tolerates_malformed_entries() {
    let mut state = all_complete_state();
    // Wrong type — a string where an array is expected. The phase
    // is skipped, not panicked on.
    state["phases"]["flow-review"]["agents_skipped"] = json!("not-an-array");
    // Entries missing `agent` / `reason` render with an empty string
    ...
    state["phases"]["flow-learn"]["agents_skipped"] = json!([
        ...
        {
            "agent": "documentation",
            "timestamp": "2026-01-01T00:46:00-08:00",
        },
    ]);
    ...
    assert!(result.summary.contains("Skipped Agents"));
    // Missing `agent` → empty agent, reason still rendered.
    assert!(result.summary.contains(" — rate_limit (Learn)"));
    // Missing `reason` → empty reason, agent still rendered.
    assert!(result.summary.contains("documentation —  (Learn)"));
}
